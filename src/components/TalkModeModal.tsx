...
const TalkModeModal: React.FC<TalkModeModalProps> = ({
      ...
      case 'error':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <button
              onClick={handleRetry}
              className="mt-2 inline-flex items-center justify-center rounded-md bg-zinc-800 px-4 py-2 text-sm font-medium text-zinc-100 transition-colors hover:bg-zinc-700"
            >
              Tentar novamente
            </button>
          </div>
      ...
      case 'recording':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <h2 className="text-lg font-semibold text-zinc-100">Gravando...</h2>
            <p className="text-sm text-zinc-400">Fale no seu ritmo. Vamos organizar tudo para você no final.</p>
          </div>
        );
      case 'transcribing':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            <div className="w-16 h-16 rounded-full bg-zinc-800 flex items-center justify-center border border-white/10">
              <SpinnerIcon className="w-7 h-7 text-zinc-200 animate-spin" />
            </div>
            <h2 className="text-lg font-semibold text-zinc-100">Transcrevendo com Gemini...</h2>
            <p className="text-sm text-zinc-400">Convertendo seu áudio em texto para analisarmos com mais clareza.</p>
          </div>
        );
      case 'analyzing':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <h2 className="text-lg font-semibold text-zinc-100">Analisando sua nota...</h2>
            <p className="text-sm text-zinc-400">Estamos classificando cada intenção e sugerindo o melhor destino.</p>
          </div>
        );
      case 'review':
        return (
          <div className="flex flex-col items-stretch text-left space-y-4 w-full">
            <div className="flex flex-col gap-2">
              <div>
                <h2 className="text-lg font-semibold text-zinc-100">Revise antes de salvar</h2>
                <p className="text-sm text-zinc-400">
                  Ajuste o que preferir manter. Cada item será enviado para o destino correto automaticamente.
                </p>
              </div>
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/5 bg-zinc-900/40 px-4 py-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
                  {Object.entries(summaryStats.groupedTotal).map(([type, totalCount]) => {
                    ...
                    return (
                      <span
                        key={type}
                        className="rounded-full border border-white/10 bg-zinc-900/60 px-3 py-1 text-zinc-300"
                      >
                  ...
                  {summaryStats.selected === 0 && (
                    <span className="text-zinc-500">Nenhum item selecionado</span>
                  )}
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <button
                    onClick={handleSelectAll}
                    className="rounded-md border border-white/10 px-3 py-1 text-zinc-300 transition-colors hover:text-zinc-100"
                  >
                    Selecionar tudo
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="rounded-md border border-white/10 px-3 py-1 text-zinc-300 transition-colors hover:text-zinc-100 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Limpar seleção
                  </button>
                </div>
                  ...
                  <button
                    ...
                    className={`w-full text-left rounded-xl border px-4 py-3 transition-all ${
                      isSelected
                        ...
                        : 'border-zinc-900 bg-zinc-950/40 opacity-70 hover:opacity-90'
                    }`}
                  >
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-zinc-900/70 px-2.5 py-1 text-xs text-zinc-300">
                              <Icon className="h-3.5 w-3.5" />
                              {item.type}
                            </span>
                            {item.dueDate && (
                              <span className="inline-flex items-center rounded-full border border-zinc-700 bg-zinc-900/80 px-2 py-0.5 text-xs text-zinc-300">
      ...
      case 'submitting':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <h2 className="text-lg font-semibold text-zinc-100">Salvando itens...</h2>
            <p className="text-sm text-zinc-400">Estamos distribuindo os itens nos locais corretos.</p>
          </div>
        );
      case 'done':
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <h2 className="text-lg font-semibold text-zinc-100">Itens criados!</h2>
            <div className="w-full rounded-md border border-white/5 bg-zinc-900/40 px-4 py-3 text-left text-sm text-zinc-400">
      ...
      default:
        return (
          <div className="flex flex-col items-center text-center space-y-3 w-full">
            ...
            <h2 className="text-lg font-semibold text-zinc-100">Preparando microfone...</h2>
            <p className="text-sm text-zinc-400">Conceda acesso ao microfone para começar a registrar sua nota falada.</p>
          </div>
        );
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div className="relative z-[1001] bg-zinc-900 rounded-xl border border-white/10 w-[min(520px,90vw)] px-6 py-8 md:px-8 md:py-10 flex flex-col items-center text-center space-y-6 animate-scale-in" onClick={e => e.stopPropagation()}>
        {renderContent()}
        {status === 'recording' && (
          <div className="w-full flex justify-center">
            <button
              onClick={handleFinish}
              className="inline-flex items-center justify-center rounded-md bg-zinc-800 px-5 py-2 text-sm font-medium text-zinc-100 transition-colors hover:bg-zinc-700"
            >
              Concluir
            </button>
          </div>
        ...
        {status === 'review' && pendingItems.length > 0 && (
          <div className="w-full flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
            <button
              onClick={handleClose}
              className="inline-flex items-center justify-center rounded-md border border-white/10 px-4 py-2 text-sm font-medium text-zinc-300 transition-colors hover:text-zinc-100"
            >
              Cancelar
            </button>
            <button
              ...
              className="inline-flex items-center justify-center rounded-md bg-zinc-100 px-4 py-2 text-sm font-semibold text-zinc-900 transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"
            >
              Aprovar e criar
            </button>
          </div>
